    ...
    <div className="bi-page">
      ...
      {step === 3 && (
        <VerifyStep
          ...
          onFinish={() => {
            reset();
            window.dispatchEvent(new CustomEvent("fe-toast", {detail: "Import complete · 847 data entries created"}));
            onFinish && onFinish();
          }}
        />
      )}
  ...
  return (
    <div className="bi-home">
      <div className="bi-home-head">
        <div>
          <h1 className="page-title">Bulk imports</h1>
          <div className="page-subtitle">Upload large datasets via CSV or XLSX. Track progress and review past imports below.</div>
        </div>
        <button className="btn primary" onClick={() => { localStorage.removeItem("fe-import-step"); localStorage.removeItem("fe-import-file"); localStorage.removeItem("fe-import-type"); setMode("wizard"); }}>
          <Icon name="plus" size={14}/>New bulk import
        </button>
      </div>
              ...
              <div key={b.id} className="bi-log-row">
                ...
                <div>
                  {b.status === "processing" ? (
                    ...
                    <button className="bi-log-link" title={`View data entries for ${b.id}`}
    ...
    return (
      <>
        <div className="bi-head">
          ...
          <div className="bi-head-sub">All {TOTAL_ROWS.toLocaleString()} rows passed validation. Finish the import to create data entries. You'll match emission factors next, over on the Calculations page.</div>
        </div>
        <div className="bi-clean">
          ...
          <div className="bi-clean-next">
            ...
            <ol>
              <li>Data entries are created for all {TOTAL_ROWS.toLocaleString()} rows.</li>
              <li>The AI matching engine suggests an emission factor for each entry.</li>
