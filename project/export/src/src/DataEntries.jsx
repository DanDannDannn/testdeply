...
function DataEntries({ entries, calcs, selectedId, setSelectedId, onViewCalc, preselectEntryId, onViewImport, bulk, batchFilter, onClearBatchFilter }) {
  ...
  const filteredEntries = React.useMemo(() => {
    ...
    return r;
  }, [entries, query, bu, cat, batchFilter, colFilters, sort, calcs]);
  ...
  return (
    <>
      <div className="page-head">
        <div>
          <h1 className="page-title">Data entries</h1>
          <div className="page-subtitle">Activity records · {entries.length} entries in Q1 2026</div>
        </div>
      ...
      <div className="filter-bar">
        {batchFilter && (
          <span className="filter active" style={{background:"var(--fe-primary-50, rgba(99,102,241,0.1))", color:"var(--fe-primary-600)"}} onClick={onClearBatchFilter} title="Clear batch filter">
            <Icon name="upload" size={14}/>Batch: {batchFilter} <Icon name="close" size={12}/>
          </span>
        )}
        <span className="filter active">All <span style={{color:"var(--fe-fg-muted)"}}>{entries.length}</span></span>
        <span className={`filter ${cat!=="all"?"active":""}`} onClick={() => {
          const cats = ["all","electricity","natural_gas","diesel","flight","purchased_goods"];
          const i = cats.indexOf(cat); setCat(cats[(i+1) % cats.length]);
        }}>
          <Icon name="filter" size={14}/>{cat === "all" ? "Category" : (<CatLabel cat={cat}/>)}
        </span>
      ...
      <aside className="drawer" role="dialog" aria-label="Entry detail">
        <div className="drawer-head">
          <div style={{flex:1, minWidth:0}}>
            <div className="kicker">Data entry · {entry.id}</div>
