...
function Calculations({ calcs, entries, selectedId, setSelectedId, filter, setFilter, onViewEntry, bulk, preset, embedded, headerPortal }) {
  ...
  }, [calcs, bu, scopeF, catF, query, period, chartSpec]);
  ...
  return (
    <>
      ...
      {chartSpec && chartRows && chartRows.length > 0 && (
        <div className="ai-context-strip" role="region" aria-label="Forward AI insight chart">
          ...
          <div className="ai-context-strip__chart">
            {chartSpec.kind === "static-delta"
              ...
              : <HorizBarChart rows={chartRows} unit={chartSpec.unit || "t"}/>
            }
          </div>
          <button
            className="ai-context-strip__close"
            onClick={() => setChartDismissed(true)}
            title="Hide insight chart"
            aria-label="Hide insight chart"
          >
      ...
      <div className="filter-bar">
        {/* All count — passive indicator of the working set */}
        <span className="filter active">
          All <span style={{color:"var(--fe-fg-muted)"}}>{counts.all}</span>
        </span>

        {/* Status — combined into a single filter pill (was three chips).
            Bound to colFilters.status so column-header and pill are one source of truth. */}
        <FilterPill
          icon="filter"
          label="Status"
